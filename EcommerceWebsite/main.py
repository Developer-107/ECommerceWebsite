...
def pay_with_paypal():
    ...
    payment = paypalrestsdk.Payment({
        ...
        "redirect_urls": {
            "return_url": "http://localhost:5001/paypal/return",
            "cancel_url": "http://localhost:5001/paypal/cancel"
        },
        "transactions": [{
            "item_list": {"items": [{"name": "Item", "sku": "item", "price": amount, "currency": "USD", "quantity": 1}]},
...
def pay_with_stripe():
    data = request.json
    amount = int(float(data['amount']) * 100)  # Stripe requires amount in cents
    currency = "usd"
    token = data['token']
    try:
